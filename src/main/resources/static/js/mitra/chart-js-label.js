...
(function () {
  ...
  if ("undefined" === typeof Chart) console.error("Can not find Chart object.");
  else {
    "function" != typeof Object.assign &&
      (Object.assign = function (a, c) {
        ...
        for (var b = Object(a), e = 1; e < arguments.length; e++) {
          ...
          if (null != d)
            for (var g in d)
              Object.prototype.hasOwnProperty.call(d, g) && (b[g] = d[g]);
        }
        return b;
      });
    var k = {};
    ...
    f.prototype.renderToElement = function (a, c, b, e) {
      if (
        this.shouldRenderToElement(c.meta, b) &&
        ((this.percentage = null), (c = this.getLabel(a, b, e)))
      ) {
        var d = this.ctx;
        d.save();
        d.font = Chart.helpers.fontString(
        ...
        this.drawable(b, c, g) &&
          (d.beginPath(),
          (d.fillStyle = this.getFontColor(a, b, e)),
          this.renderLabel(c, g));
        d.restore();
      }
    };
    ...
    f.prototype.renderBaseLabel = function (a, c) {
      ...
      if ("object" === typeof a)
        ...
        this.options.textShadow &&
          ((b.shadowOffsetX = this.options.shadowOffsetX),
          (b.shadowOffsetY = this.options.shadowOffsetY),
          (b.shadowColor = this.options.shadowColor),
          (b.shadowBlur = this.options.shadowBlur));
        for (var e = a.split("\n"), d = 0; d < e.length; d++)
          b.fillText(
            e[d],
            c.x,
            c.y -
              (this.options.fontSize / 2) * e.length +
              this.options.fontSize * d
          );
        b.restore();
      }
    };
    f.prototype.renderArcLabel = function (a, c) {
      ...
      if ("string" === typeof a) {
        ...
        "border" === this.options.position &&
          (f = ((d.length - 1) * this.options.fontSize) / 2);
    ...
    f.prototype.getLabel = function (a, c, b) {
      if ("function" === typeof this.options.render)
        a = this.options.render({
          label: this.chart.config.data.labels[b],
          value: a.data[b],
        ...
        switch (this.options.render) {
          case "value":
            ...
            break;
          case "label":
            a = this.chart.config.data.labels[b];
            break;
          case "image":
            a = this.options.images[b]
              ? this.loadImage(this.options.images[b])
              : "";
            break;
          default:
            a = this.getPercentage(a, c, b) + "%";
        }
      "object" === typeof a
        ? (a = this.loadImage(a))
    ...
    f.prototype.getFontColor = function (a, c, b) {
      ...
      "function" === typeof e
        ...
        : "string" !== typeof e &&
          (e = e[b] || this.chart.config.options.defaultFontColor);
      return e;
    };
    f.prototype.getPercentage = function (a, c, b) {
      ...
      if ("polarArea" === this.chart.config.type) {
        ...
        if (void 0 === this.barTotal[b])
          for (
            c = this.barTotal[b] = 0;
            c < this.chart.data.datasets.length;
            ++c
          )
            this.barTotal[b] += this.chart.data.datasets[c].data[b];
        a = (a.data[b] / this.barTotal[b]) * 100;
      } else
        a =
          (c._view.circumference / this.chart.config.options.circumference) *
          100;
      a = parseFloat(a.toFixed(this.options.precision));
      this.options.showActualPercentages ||
        ("bar" === this.chart.config.type &&
          (this.totalPercentage = this.barTotalPercentage[b] || 0),
        (this.totalPercentage += a),
        100 < this.totalPercentage &&
          ((a -= this.totalPercentage - 100),
          (a = parseFloat(a.toFixed(this.options.precision)))),
        "bar" === this.chart.config.type &&
          (this.barTotalPercentage[b] = this.totalPercentage));
    ...
    f.prototype.getBaseRenderInfo = function (a, c) {
      if (
        ...
        "border" === this.options.position
          ...
          : "outside" === this.options.position &&
            (b = e.outerRadius - g + g + this.options.textMargin);
        b = { x: e.x + Math.cos(d) * b, y: e.y + Math.sin(d) * b };
        "outside" === this.options.position &&
    ...
    f.prototype.drawable = function (a, c, b) {
      ...
      if (this.options.arc) return b.endAngle - b.startAngle <= b.totalAngle;
    ...
    f.prototype.outsideInRange = function (a, c, b, e) {
      for (var d = this.labelBounds, g = 0; g < d.length; ++g) {
        ...
        for (h = 0; h < k.length; ++h)
          if (
            ((m = k[h][0]), (n = k[h][1]), m >= a && m <= c && n >= b && n <= e)
          )
            return !1;
      }
    ...
    f.prototype.measureLabel = function (a) {
      if ("object" === typeof a) return { width: a.width, height: a.height };
      var c = 0;
      a = a.split("\n");
      ...
      return { width: c, height: this.options.fontSize * a.length };
    };
    ...
    Chart.plugins.register({
      ...
      beforeDatasetsUpdate: function (a, c) {
        if (k[a.config.type]) {
          Array.isArray(c) || (c = [c]);
          var b = c.length;
          (a._labels && b === a._labels.length) ||
            (a._labels = c.map(function () {
              return new f();
            }));
          for (var e = !1, d = 0, g = 0; g < b; ++g) {
            var l = a._labels[g];
            l.setup(a, c[g]);
            "outside" === l.options.position &&
              ((e = !0),
              (l = 1.5 * l.options.fontSize + l.options.outsidePadding),
              l > d && (d = l));
          }
